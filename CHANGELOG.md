...
**Technical details.**
- `system.py` — `on_click_system_firmware` / `_remove` mirror the existing GParted Launch/Install/Remove handlers; `_launch_firmware` reuses `_partition_tool_launch_cmd("gnome-firmware")` (real-user launch with XDG/DBus/DISPLAY). Timer handlers (`on_click_system_firmware_timer_enable` / `_disable`) call `systemctl enable/disable --now fwupd-refresh.timer` directly in a daemon thread — **not** `fn.enable_service()`/`disable_service()`, which hardcode-append `.service` and would corrupt the timer unit name. `_refresh_firmware_timer_label` reads `systemctl is-enabled` live to avoid the cached `check_service_enabled`. Removing GNOME Firmware now uses `pacman -Rns` (via a new `op` parameter on `launch_pacman_remove_in_terminal`, default `-R` so every other Remove button is unchanged) so `fwupd` and its `fwupd-refresh.timer` go too when nothing else needs them — `-s`/`-n` are safe per pacman(8) (skip still-required/explicit packages, skip `.pacsave`). As a fallback for when `fwupd` was explicitly installed and survives, the removal still disables + stops `fwupd-refresh.timer` (guarded on `_firmware_timer_enabled()`, no-op when never enabled) and refreshes the timer label — previously the timer kept running after the package was gone.
