...
# is snapshot-ready but inert: no snapper stack, no config. This page installs
# and configures that stack opt-in, visibly (no black boxes), with Kiro's
# policy of snap-pac + cleanup but NO hourly timeline.
...
def on_click_enable_snapshots(self, _widget):
    """Install + configure the snapshot stack in a visible Alacritty window."""
    ...
    def wait_and_refresh():
        ...
        fn.GLib.idle_add(_refresh_after_setup, self)
...
def _refresh_after_setup(self):
    ...
    fn.show_in_app_notification(self, "Snapshot setup complete")
    return False
...
def on_click_launch_assistant(self, _widget):
    ...
    if not fn.check_package_installed("btrfs-assistant"):
        ...
        fn.show_in_app_notification(self, "Install the snapshot tools first")
        return
